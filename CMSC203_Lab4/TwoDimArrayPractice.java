...
public class TwoDimArrayPractice extends JFrame
{
   ...
   */
   public int findMinimum( int column )
   {
    ...
    // Write your code here:
	   int minimumValue = intArray[0][column];
	   for(int row = 1; row < intArray.length;row++) {
		   if(minimumValue > intArray[row][column]) {
			   minimumValue = intArray[row][column];
		   }
		   animate(row, column, minimumValue);
	   }

      return minimumValue; // replace this line with your return statement
   }
   ...
   */
   public int countFound( int value )
   {
    ...
    // Write your code here:
	   ...
	   for(int row = 0; row < intArray.length; row++) {
		   for(int col = 0; col < intArray[row].length; col++) {
			   if(value == intArray[row][col]) {
				   countValue++;
			   }
			   animate(row,col,countValue);
		   }
	   }

     return countValue; // replace this line with your return statement
   }
